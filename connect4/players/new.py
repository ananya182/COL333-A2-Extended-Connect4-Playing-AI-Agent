    ...
    def simulate_board(self, column: int, player_num: int, is_popout: bool, board, num_popouts):
        ...
        if not is_popout:
            ...
            if 1 in board[:, column] or 2 in board[:, column]:
                ...
                raise Exception(err)
# -------------------------------------------------------------------------------------------------------------------------------- 
    def eval_function(self, state, player_num, total_moves):
    ...
    def min_value(self, state, total_moves, depth, limit, alpha, beta, start):
        try:
            if depth >= limit:
                return self.eval_function(state, self.player_number, total_moves), state[0]
            ...
            for play_move in valid_actions:
                ...
                #     # time.sleep(0.1)
                #     print("\n")
                cur_val, myboard = self.max_value(state_new, total_moves-1, depth+1, limit, alpha, beta, start) #simulating the move of the next player
    ...
    def max_value(self, state,total_moves, depth, limit, alpha, beta, start):

        try:
            if depth >= limit:
                return self.eval_function(state,self.player_number,total_moves), state[0]
    
            valid_actions = get_valid_actions(self.player_number, state) #maximizing for our own player
            max_val = -1 * np.inf
            out_board = state[0]
            ...
            for play_move in valid_actions:
                ...
                cur_val, myboard = self.min_value(state_new, total_moves-1, depth+1, limit, alpha, beta, start)
                # state_new = (board_new,num_popout_new) 
        ...
        while True:
            ...
            try :
                ...
                for play_move in valid_actions:
                    ...
                    new_min, myboard = self.min_value(state_new, total_moves, 1, limit, -np.inf, np.inf, start)
